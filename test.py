...
for idx_line in range(len(df)):
    ...
    # 한 글자 이하의 단어는 제외하고, 공백도 제거
    nouns_list_c = [nouns for nouns in nouns_list if len(nouns) > 1 and nouns.strip() != '']
    print(nouns_list_c)
    df.loc[idx_line, 'keyword'] = " ".join(nouns_list_c)  # 공백으로 구분된 문자열로 할당
    print(df.loc[idx_line, 'keyword'])
